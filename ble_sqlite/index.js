...
function handleUserPrompt(prompt) {
  ...
  //chatLog.log(`{italic}{yellow-fg}Codex is writing code...{/yellow-fg}{/italic}`);
  chatLog.log(`{yellow-fg}App writing text...{/yellow-fg}`);
  screen.render();
  ...
  setTimeout(async() => {
    ...
    const resp_greet = myGreet(prompt);
    let target = prompt;
    if (target.startsWith("add ")) {
      target = target.replace(/add /g, "");
      todoAdd(target); 
      chatLog.log(`\n{bold}App reply:{/bold} ${target}`);
    }
    if (target.startsWith("list")) {
      ...
      items.forEach((element) => {
        chatLog.log(`--------------------------------------------------------------------------------`);
        chatLog.log(`id= ${element.id} ${element.title}`);
      });
      //chatLog.log(`\n{bold}App reply:{/bold} ${resp}`);
    }
    
    if (response.code) {
      chatLog.log(`\n {yellow-fg}[updated chat history]{/yellow-fg}`);      
    }
